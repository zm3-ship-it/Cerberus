...
const doRecon=()=>{setRcing(true);setAps([]);setTAP(null);setCli([]);setSel(new Set());setMS(new Set());setDS(new Set());setDns([]);setDi(0);setPrb([]);setHsState("idle");let i=0;const iv=setInterval(()=>{if(i<APS.length){setAps(p=>[...p,APS[i]]);i++;}else{clearInterval(iv);setRcing(false);}},300);};
const doScan=()=>{if(!tAP)return;setScing(true);setCli([]);setSel(new Set());setMS(new Set());setDS(new Set());setDns([]);setDi(0);setPrb([]);const pool=CLI_MAP[tAP.ssid]||[];let i=0;let pi=0;const iv=setInterval(()=>{if(i<pool.length){setCli(p=>[...p,pool[i]]);i++;if(pi<PRB.length&&Math.random()>0.4){setPrb(p=>[...p,PRB[pi]]);pi++;}}else{clearInterval(iv);setScing(false);}},400);};
