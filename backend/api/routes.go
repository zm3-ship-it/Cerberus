...
func NewRouter(s *scanner.Scanner, m *mitm.Engine, d *deauth.Engine, e *eviltwin.Engine, c *captive.Engine, h *handshake.Engine, a *adapters.Manager, n *network.Manager) http.Handler {
	...
	mux.HandleFunc("/api/adapters/role", w(func(rw http.ResponseWriter, rq *http.Request) { var a struct{ Adapter, Role string }; dec(rq, &a); r.adpt.SetRole(a.Adapter, a.Role); j(rw, M{"status": "ok"}) }))
